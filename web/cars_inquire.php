...
<main class="containerin">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" class="form-containerin">
        ...
        <label for="repair_status" class="form-label-inlinein">Cars in Repair</label><br>

        <input type="checkbox" id="damage_status" name="damage_status" <?= $damageStatus ? 'checked' : '' ?> class="form-checkboxin">
        <label for="damage_status" class="form-label-inlinein">Cars in Damage</label><br>
        ...
        <tbody>
            <?php foreach ($cars as $car): ?>
                <tr>
                    ...
                    <td><img src="<?= $car['photo_filename'] ?>" alt="<?= $car['car_model'] ?>" style="max-width: 100px;"></td>
                    <td><?= $car['fuel_type'] ?></td>
